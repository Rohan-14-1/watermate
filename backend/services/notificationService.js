...
class NotificationService {
  /**
   * Synchronizes and notifies the currently active turn member.
   * Completely idempotent: uses WaterTurnNotificationLog so duplicate calls
   * (e.g. from completeTurn, advanceTurn, page reloads, app opening) never send duplicate notifications.
   * @param {string} groupId
   * @param {Object} [options]
   */
  async syncAndNotifyActiveTurn(groupId, options = {}) {
    ...
    try {
      ...
      const turnEventKey = options.eventKey || `active_${groupId}_${activeMember.userId}_${todayKey}`;

      // Check if notification has already been logged for this turn event
      const existingLog = await prisma.waterTurnNotificationLog.findUnique({
        where: {
          groupId_userId_turnEventKey_notificationType: {
            groupId,
            userId: activeMember.userId,
            turnEventKey,
            notificationType: "ACTIVE_TURN",
          },
        },
      });

      if (existingLog) {
        return null; // Already notified for this turn event
      }

      // Record log entry (unique constraint protects against race conditions)
      try {
        await prisma.waterTurnNotificationLog.create({
          data: {
            groupId,
            userId: activeMember.userId,
            turnEventKey,
        ...
        if (logErr?.code === "P2002") return null; // Concurrent insertion
        throw logErr;
      }

      // Create in-app Notification record in PostgreSQL
      const title = "💧 Your Water Turn";
      const body = `It's your turn to bring water for ${activeMember.group?.name || "your flat"} today.`;
      ...
      const notification = await prisma.notification.create({
        data: {
          ...
          title,
          body,
          referenceId: turnEventKey,
        },
      });

      // Dispatch push notification
      await pushService.sendToUser(activeMember.userId, {
        title,
        body,
        data: {
          type: "WATER_TURN",
      ...
      console.warn("[NotificationService] syncAndNotifyActiveTurn error:", err.message);
      return null;
    }
  }

  /**
   * Sends an upcoming turn reminder (e.g. tomorrow) idempotently.
   * @param {string} groupId
   * @param {string} nextUserId
   */
  async notifyUpcomingTurn(groupId, nextUserId) {
    if (!groupId || !nextUserId) return null;

    try {
      const todayKey = new Date().toISOString().slice(0, 10);
      const turnEventKey = `upcoming_${groupId}_${nextUserId}_${todayKey}`;
      ...
      try {
        ...
        if (logErr?.code === "P2002") return null;
        throw logErr;
      }

      const title = "💧 Water Turn Reminder";
      const body = "Your turn to bring water is coming up tomorrow.";
      ...
      await pushService.sendToUser(nextUserId, {
        title,
        body,
        data: { type: "UPCOMING_REMINDER", groupId, notificationId: notification.id },
      });

      return notification;
    } catch (err) {
      console.warn("[NotificationService] notifyUpcomingTurn error:", err.message);
      return null;
    }
  }
